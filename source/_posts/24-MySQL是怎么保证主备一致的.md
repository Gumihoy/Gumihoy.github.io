...
• 不过一般出现循环复制的情况，往往线上同时还有更新，这时就不能修改 server_id 了。这种情况下，我们需要先在节点 A 上执行 stop slave 命令，先停止同步；然后手动执行 change master 命令，指向 A’最后一个 binlog 文件的文件末尾，再 start slave 就可以结束循环复制了。